...
def test_diagnosis_flow_ranks_anthracnose_first() -> None:
    ...
    assert app.success[0].value == "**Interpretasi Sistem:** Sangat Kuat"

    ranking_df = next(df.value for df in app.dataframe if "Ranking" in df.value.columns)
    ...
    rule_df = next(df.value for df in app.dataframe if "Rule ID" in df.value.columns)
    assert set(rule_df["Rule ID"]) == {"R-P1-CORE", "R-P1-SIGNAL"}

    history_df = next(df.value for df in app.dataframe if "Waktu" in df.value.columns)
...
def test_home_page_renders_metrics_and_guidance() -> None:
    ...
    assert metrics == {
        "Jumlah Penyakit": "7",
        "Jumlah Gejala": "25",
        "Output Sistem": "Excel .xlsx",
    }
    assert "halaman `Diagnosis`" in app.info[0].value
...
def test_info_penyakit_page_lists_all_diseases() -> None:
    ...
    assert app.caption[0].value.startswith("Edukasi dan ringkasan penyakit utama")
...
def test_about_page_shows_method_and_disclaimer() -> None:
    ...
    assert "hasil komputasi tidak menggantikan" in app.warning[0].value.lower()
